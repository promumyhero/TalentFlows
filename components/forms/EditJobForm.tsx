...
export function EditJobForm({ editJob }: EditJobFormProps) {
  const form = useForm<z.infer<typeof jobSchema>>({
    resolver: zodResolver(jobSchema),
    defaultValues: {
      benefits: editJob.benefits,
      companyAbout: editJob.Company?.about,
      companyLocation: editJob.Company?.location,
      companyName: editJob.Company?.name,
      companyLogo: editJob.Company?.logo,
      companyWebsite: editJob.Company?.website,
      companyXAccount: editJob.Company?.xAccount || "",
      employementType: editJob.employementType,
      jobDescription: editJob.jobDescription,
      jobTitle: editJob.jobTitle,
      listingDuration: editJob.listingDuration, // nanti ganti jadi number
      location: editJob.location,
      salaryFrom: editJob.salaryFrom, // nanti ganti jadi number
      salaryTo: editJob.salaryTo, // nanti ganti jadi number
    },
  });
  ...
  async function onSubmit(values: z.infer<typeof jobSchema>) {
    try {
      setPending(true);
      await editJobPost(values, editJob.id);
    } catch (error) {
